 ...
 * to assert on it.
 */
class PostgresContainer(imageName: String) :
    ...
    private fun initializeDataSource() {
        val config =
            HikariConfig()
                .apply {
                    jdbcUrl = this@PostgresContainer.jdbcUrl
                    username = this@PostgresContainer.username
                    password = this@PostgresContainer.password
                    driverClassName = this@PostgresContainer.driverClassName
                }
...
/** Returns a new [PostgresContainer] with which a PostgreSQL instance is run. */
internal fun ostgresContainer(): PostgresContainer =
    PostgresContainer("postgres:13.1-alpine")
